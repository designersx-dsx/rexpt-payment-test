...
import { createNumberOrder, fetchAvailablePhoneNumberByCountry, updateAgent } from '../../Store/apiStore';
...
const AssignNumber = () => {
    ...
    useEffect(() => {
        if (countryCode || cityName || stateCode) {
            setLoading(true)
            fetchAvailablePhoneNumberByCountry(countryCode, cityName, stateCode)
                .then((res) => {
    ...
    const handleRefresh = () => {
        ...
        fetchAvailablePhoneNumberByCountry(countryCode, cityName, stateCode, startsWith, endsWith)
                ...
                <Modal2 isOpen={isModalOpen} onClose={() => setModalOpen(false)}>
                    <div className={styles.modalContent}>
                        ...
                        <div className={styles.assignBtn}>
                            <AnimatedButton isLoading={loading} label="Assign Number" onClick={handleBuyNumber} position={{ position: 'relative' }} />
                        </div>
                    </div>
                </Modal2>
            </div>
