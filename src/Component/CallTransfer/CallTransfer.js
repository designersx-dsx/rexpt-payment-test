...
function CallTransfer() {
    ...
    const [transfers, setTransfers] = useState([{ condition: '', phone: '', dialCode: '91', countryCode: 'in' }]);
    ...
    const handleSubmit = async () => {
        try {
            ...
            const dynamicVars = {
                sales_number: salesEntry ? `+${salesEntry.dialCode}${salesEntry.phone}` : '',
                billing_number: billingEntry ? `+${billingEntry.dialCode}${billingEntry.phone}` : '',
                support_number: supportEntry ? `+${supportEntry.dialCode}${supportEntry.phone}` : ''
            };
    ...
    useEffect(() => {
        ...
        const agentGeneralTools = JSON.parse(sessionStorage.getItem("agentGeneralTools"))
        ...
        if (Array.isArray(agentGeneralTools)) {
            ...
            if (Array.isArray(agentGeneralTools) && agentGeneralTools.length > 0) {
                ...
                const filteredTransfers = agentGeneralTools
                    .filter(tool =>
                        tool.condition && tool.phone && tool.dialCode
                    )
                    .map(tool => ({
        ...
        <>
            ...
            <div className={styles.CallTransferMain}>
                <div className={styles.headrPart}>
                    ...
                    <img src='svg/Add-icon.svg' alt='Add-icon' onClick={handleAdd} style={{ cursor: 'pointer' }} />
                </div>
                ...
                {transfers.map((item, index) => (
                    <div key={index} className={styles.card}>
                            ...
                            <div className={styles.countryCode}>
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        updated[index].countryCode = dialToCountry[value] || updated[index].countryCode;
                                        setTransfers(updated);
                                    }}
                        ...
                        {transfers.length > 1 && (
                            <button onClick={() => handleRemove(index)} className={styles.removeBtn}>
                                Remove
                            </button>
                        )}
                    </div>
